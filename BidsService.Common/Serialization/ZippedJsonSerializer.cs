...
/// </summary>
public class ZippedJsonSerializer : ISerializer
{
    /// <summary>
    ...
    public Stream Serialize<T>(T value)
    {
        ...
        if (value != null)
        {
            using (var compressedStream = new DeflateStream(buffer, CompressionMode.Compress, true))
            using (var writer = new StreamWriter(compressedStream))
            {
                writer.Write(JsonConvert.SerializeObject(value, value.GetType(), Formatting.None, this.SerializerSettings));
            }
    ...
    {
        using var compressedStream = new DeflateStream(stream, CompressionMode.Decompress, true);
